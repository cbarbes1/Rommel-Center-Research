...
class TestMinHashUtility(unittest.TestCase):
    ...
    def test_signature_comparisoin(self):
        ...
        self.assertTrue(
            all(isinstance(num, (int, float)) for num in signature1),
            "Signature1 contains non-numeric values",
        )
        self.assertTrue(
            all(isinstance(num, (int, float)) for num in signature2),
            "Signature2 contains non-numeric values",
        )
        similarity = self.minhash_util.compare_signatures(signature1, signature2)
        self.assertTrue(
            0 < similarity <= 1
        )  # expect some similarity but not necessarily 1
