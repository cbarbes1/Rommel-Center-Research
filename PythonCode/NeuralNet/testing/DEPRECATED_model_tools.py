...
class ModelTools:
    def __init__(self, filename: str, batch_size: int = 32, 
                 max_vocab_size: int = 11000, max_length: int = 500, 
        ...
        if use_word_embeddings:
            self.embeddings_layer = hub.KerasLayer(USE_MODEL_URL, input_shape=[], 
                                                   dtype=tf.string, trainable=True)
        
        self.text_vectorization = TextVectorization(max_tokens=max_vocab_size, output_sequence_length=max_length)
    ...
    def load_and_preprocess_data(self) -> None:
        ...
        # Apply batching, shuffling, and prefetching to optimize training
        self.train_dataset = self.train_dataset.shuffle(buffer_size=len(train_texts)).batch(self.batch_size).prefetch(tf.data.AUTOTUNE)
        self.val_dataset = self.val_dataset.batch(self.batch_size).prefetch(tf.data.AUTOTUNE)
        self.test_dataset = self.test_dataset.batch(self.batch_size).prefetch(tf.data.AUTOTUNE)
...
if __name__ == "__main__":
    ...
    text_model = TextClassificationModel(num_classes=num_classes, 
                                         ...
                                         max_features=11000, sequence_length=500)
