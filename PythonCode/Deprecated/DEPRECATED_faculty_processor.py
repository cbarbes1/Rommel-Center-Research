...
class FacultyPostprocessor:
    ...
    def filter_names_by_initials(self, author_set):
        ...
        filtered_set = {
            ...
            if len(name.split()) >= 2
            and name.split()[0][0].lower() == name.split()[-1][0].lower()
        }
        return filtered_set
    ...
    def duplicate_postprocessor(
        ...
        name_frequency = {
            name: sum(name in f_set for f_set in faculty_sets) for name in all_names
        }
        ...
        for i in range(len(names_list)):
            for j in range(i + 1, len(names_list)):
                ...
                if similarity > similarity_threshold:
                    if name_frequency[name1] > name_frequency[name2] or (
                        name_frequency[name1] == name_frequency[name2] and name1 < name2
                    ):
                        to_remove.add(name2)
                    else:
                        to_remove.add(name1)
    ...
    def duplicate_postprocessor1(
        ...
        to_remove = {
            name for name, decision in final_decision.items() if decision == "remove"
        }
